...
static struct proc_dir_entry *cdt_proc;
static struct proc_dir_entry *lcdid_proc;
...
#define CDT_PROC_FILE   "cdt"
extern int g_cdt;

static ssize_t cdt_proc_read(struct file *filp, char __user *buf,
...
static const struct file_operations cdt_proc_ops = {
	.owner = THIS_MODULE,
	.read = cdt_proc_read,
};

static int create_cdt_file(struct platform_device *pdev)
{
	/* Create proc file system */
	cdt_proc = NULL;
	cdt_proc = proc_create(CDT_PROC_FILE, 0664, NULL, &cdt_proc_ops);

	if (!cdt_proc) {
		pr_err("%s: create proc_entry %s failed\n", __func__, CDT_PROC_FILE);
		return -ENODEV;
	} else {
		pr_err("%s: create proc_entry %s success\n", __func__, CDT_PROC_FILE);
	}
...
#define GPIO_LCD_ID1	141
static unsigned char status0 = 0;
static unsigned char status1 = 0;

static ssize_t lcdid_proc_read(struct file *filp, char __user *buf,
			       size_t len, loff_t *off)
{
	int ret;
	char val[4];
...
static const struct file_operations lcdid_proc_ops = {
	.owner = THIS_MODULE,
	.read = lcdid_proc_read,
};

static int create_lcdid_file(struct platform_device *pdev)
{
	/* Create proc file system */
	lcdid_proc = NULL;
	lcdid_proc = proc_create(LCDID_PROC_FILE, 0664, NULL, &lcdid_proc_ops);

	if (!lcdid_proc) {
		pr_err("%s: create proc_entry %s failed\n", __func__, LCDID_PROC_FILE);
		return -ENODEV;
	} else {
		pr_err("%s: create proc_entry %s success\n", __func__, LCDID_PROC_FILE);
	}
	...
	int ndx)
{
	...
	status0 = gpio_get_value(GPIO_LCD_ID0);
	pr_err("%s: read GPIO_LCD_ID0 %d = %d\n", __func__, GPIO_LCD_ID0, status0);

	status1 = gpio_get_value(GPIO_LCD_ID1);
	pr_err("%s: read GPIO_LCD_ID1 %d = %d\n", __func__, GPIO_LCD_ID1, status1);
...
static int mdss_dsi_ctrl_probe(struct platform_device *pdev)
{
	...
	rc = create_cdt_file(pdev);
	if (rc) {
		pr_err("%s: unable to create cdt file\n", __func__);
	}

	rc = create_lcdid_file(pdev);
	if (rc) {
		pr_err("%s: unable to create lcdid file\n", __func__);
	}
